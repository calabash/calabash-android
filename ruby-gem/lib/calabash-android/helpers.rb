...
def main_activity(app)
  begin
    ...
    manifest_data = `"#{Calabash::Android::Environment.tools_dir}/aapt" dump xmltree "#{app}" AndroidManifest.xml`
...
def unsign_apk(path)
  files_to_remove = `"#{Calabash::Android::Environment.tools_dir}/aapt" list "#{path}"`.lines.collect(&:strip).grep(/^META-INF\//)
  if files_to_remove.empty?
    log "App wasn't signed. Will not try to unsign it."
  else
    system("\"#{Calabash::Android::Environment.tools_dir}/aapt\" remove \"#{path}\" #{files_to_remove.join(" ")}")
  end
end

def zipalign_apk(inpath, outpath)
  cmd = %Q("#{Calabash::Android::Environment.zipalign_path}" -f 4 "#{inpath}" "#{outpath}")
