  ...
  module Android
    module Operations
      ...
      def current_activity
        `#{default_device.adb_command} shell dumpsys window windows`.each_line.grep(/mFocusedApp.+[\.\/]([^.\s\/\}]+)/) { Regexp.last_match(1) }.first
      end
      ...
      def perform_action(action, *arguments)
        ...
        elsif deprecated_actions.key?(action)
      ...
      def each_item(opts = { query: 'android.widget.ListView', post_scroll: 0.2 }, &_block)
        uiquery = opts[:query] || 'android.widget.ListView'
        skip_if = opts[:skip_if] || lambda { |_i| false }
        stop_when = opts[:stop_when] || lambda { |_i| false }
      ...
      class Device
        ...
        def install_app(app_path)
          ...
          unless succeeded
            ...
            fail "#{pn} did not get installed. Reason: '#{result.lines.last.chomp}'. Aborting!"
          end
        end
        ...
        def update_app(app_path)
          ...
          unless succeeded
            ::Cucumber.wants_to_quit = true
            fail "#{pn} did not get updated. Aborting!"
          end
        end
        ...
        def uninstall_app(package_name)
          ...
          unless succeeded
            ::Cucumber.wants_to_quit = true
            fail "#{package_name} was not uninstalled. Aborting!"
          end
        end
        ...
        def perform_action(action, *arguments)
          ...
          Timeout.timeout(300) do
            ...
            fail 'Empty result from TestServer' if result.chomp.empty?
            result = JSON.parse(result)
            unless result['success']
              fail "Action '#{action}' unsuccessful: #{result['message']}"
            end
            result
          end
        ...
        def make_http_request(options)
          @http = init_request(options) unless @http
          ...
          fail Errno::ECONNREFUSED if response.status_code == 502
          response.body
        rescue => e
          if @http
            @http.reset_all
        ...
        def configure_http(http, options)
          ...
          if options.key?(:debug) && options[:debug]
        ...
        def screenshot(options = { prefix: nil, name: nil })
          ...
          if ENV['SCREENSHOT_VIA_USB'] == 'false'
            ...
            log screenshot_cmd
            fail 'Could not take screenshot' unless system(screenshot_cmd)
          end
        ...
        def server_version
          begin
            ...
            fail 'Invalid response' unless response['success']
          rescue => e
        ...
        def default_serial
          ...
          fail 'No connected devices' if devices.empty?
          fail 'More than one device connected. Specify device serial using ADB_DEVICE_ARG' if devices.length > 1
          devices.first
        end
        ...
        def default_server_port
          ...
          File.open(File.expand_path(server_port_configuration), File::RDWR | File::CREAT) do |f|
            ...
            return ports[serial] if ports.key?(serial)

            port = 34_777
            port += 1 while ports.value?(port)
            ports[serial] = port
        ...
        def wake_up
          ...
          log wake_up_cmd
          fail 'Could not wake up the device' unless system(wake_up_cmd)

          Retriable.retriable tries: 10, interval: 1 do
            fail 'Could not remove the keyguard' if keyguard_enabled?
          end
        end

        def clear_app_data
          cmd = "#{adb_command} shell am instrument #{package_name(@test_server_path)}/sh.calaba.instrumentationbackend.ClearAppData"
          fail 'Could not clear data' unless system(cmd)
        end

        def pull(remote, local)
          cmd = "#{adb_command} pull #{remote} #{local}"
          fail "Could not pull #{remote} to #{local}" unless system(cmd)
        end

        def push(local, remote)
          cmd = "#{adb_command} push #{local} #{remote}"
          fail "Could not push #{local} to #{remote}" unless system(cmd)
        end

        def start_test_server_in_background(options = {})
          fail 'Will not start test server because of previous failures.' if ::Cucumber.wants_to_quit

          wake_up if keyguard_enabled?
          ...
          log cmd
          fail 'Could not execute command to start test server' unless system("#{cmd} 2>&1")

          Retriable.retriable tries: 10, interval: 1 do
            fail 'App did not start' unless app_running?
          end
        ...
        def set_gps_coordinates_from_location(location)
          ...
          fail "Got no results for #{location}" if results.empty?
        ...
        def get_preferences(name)
          ...
          if app_running?
            ...
            fail 'Could not get preferences' unless system(cmd)
            ...
            fail 'Could not get preferences' unless !json.nil? and json['success']
          end
        ...
        def set_preferences(name, hash)
          ...
          if app_running?
            ...
            fail 'Could not set preferences' unless system(cmd)
            ...
            fail 'Could not set preferences' unless !json.nil? and json['success']
          end
        end
        ...
        def clear_preferences(name)
          ...
          if app_running?
            ...
            fail 'Could not clear preferences' unless system(cmd)
            ...
            fail 'Could not clear preferences' unless !json.nil? and json['success']
          end
        end
      ...
      def select_item_from_spinner(item_query_string, options = {})
        ...
        unless direction == :up || direction == :down
          fail "Invalid direction '#{direction}'. Only upwards and downwards scrolling is supported"
        end
      ...
      def swipe(_query_string, _options = {})
        fail 'Swipe not implemented. Use flick or pan instead.'
      end

      def cell_swipe(_options = {})
        ni
      end
      ...
      def scroll(query_string, direction)
        if direction != :up && direction != :down
          fail 'Only upwards and downwards scrolling is supported for now'
        end

        action = lambda do
          elements = query(query_string)
          fail "No elements found. Query: #{query_string}" if elements.empty?

          query_string = "#{query_string} index:0" if elements.length > 1

          element = elements.first

          response = query(query_string, :getFirstVisiblePosition).first

          if response.is_a?(Hash) && response.key?('error') # View is not of type android.widget.AbsListView
            scroll_x = 0
            scroll_y = 0
      ...
      def scroll_to(query_string, options = {})
        ...
        fail "No elements found. Query: #{all_query_string}" if element.nil?
        element_y = element['rect']['y']
        element_height = element['rect']['height']
        element_bottom = element_y + element_height

        scroll_view_query_string = options[:container] || if element.key?('html')
        ...
        fail "Could not find parent scroll view. Query: '#{escape_quotes(scroll_view_query_string)}'" if scroll_element.nil?
        ...
        if scroll_by_y != 0
          ...
          fail 'Could not scroll parent view' if result != '<VOID>'
        end
      ...
      def element_is_not_hidden(_uiquery)
        ni
      end

      def load_playback_data(_recording, _options = {})
        ni
      end

      def playback(_recording, _options = {})
        ni
      end

      def interpolate(_recording, _options = {})
        ni
      end
      ...
      def record_end(_file_name)
        ni
      end

      def evaluate_javascript(query_string, javascript, _opt = {})
        ...
        if result['outcome'] != 'SUCCESS' || result['results'].nil?
          ...
          fail "Could not evaluate javascript: \n#{parsed_result}"
        end

        result['results']
      end

      def backdoor(method_name, arguments = [], _options = {})
        arguments = [arguments] unless arguments.is_a?(Array)

        result = JSON.parse(http('/backdoor', method_name: method_name, arguments: arguments))

        fail result.to_s if result['outcome'] != 'SUCCESS'
