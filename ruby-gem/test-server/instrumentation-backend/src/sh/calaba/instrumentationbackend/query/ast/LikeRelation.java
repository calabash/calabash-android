...
public class LikeRelation implements UIQueryASTPredicateRelation {
	...
	/**
	 * Is firstValue LIKE secondValue?
	 * In this case secondValue is a pattern ala 'Cou* Down'
	 * which will match a string 'Count Down'.
	 */
	@Override
	public boolean areRelated(Object firstValue, Object secondValue) {
		...
		if (firstValue instanceof String && secondValue instanceof String) {
			...
			String patternStr = (String) secondValue;			
	...
	private Matcher matcherFrom(String patternStr, String stringToMatch) {
		...
		if (this.caseSensitive) {
			...
			compiledPat = Pattern.compile(translatePatternString(patternStr), Pattern.CASE_INSENSITIVE);
		}
	...
	private String translatePatternString(String patternStr) {
		...
		if (beginAt < patternStr.length()) {
			String subString = patternStr.substring(beginAt, patternStr.length());			
			converted.append(Pattern.quote(subString));	
		}
